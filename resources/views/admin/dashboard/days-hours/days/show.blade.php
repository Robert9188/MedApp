<x-admin-dsh-layout>
    ...
    <x-slot name="header">
        <span class="text-gray-500">Afficher Jours</span>
    </x-slot>

    <div class="w-full space-y-8">
        {{--   Edit Profile Button  --}}
        <div class="w-full flex justify-end items-center space-x-4 px-10">
            <x-my-link-btn link="{{ route('employee.day.edit', $day) }}" classes="bg-green-500">
                <span class="capitalize text-gray-100 shadow-xl">Editer profile</span>
            </x-my-link-btn>
            <x-my-link-btn link="{{ route('employee.day.delete', $day) }}" classes="bg-red-500">
        ...
        <div class="col-span-2 w-full bg-white rounded-md shadow-xl p-4">
            <div class="w-full bg-yellow-500 rounded shadow-md mb-5 py-1">
                <h3 class="text-gray-50 text-center">{{ $day->name }}</h3>
            </div>
            ...
            <div class="w-full flex justify-center items-center">
                <div class="w-3/4 space-y-5 bg-gray-200">
                    <div class="w-full flex justify-between items-center bg-gray-100 px-5 py-1">
                        ...
                        <h3 class="text-">{{$day->name}}</h3>
                    </div>

                    <div class="w-full flex justify-between items-center bg-gray-100 px-5 py-1">
                        <h3 class="text-">Service</h3>
                        <h3 class="text-">{{$day->service->name}}</h3>
                    </div>

                    <div class="w-full flex justify-between items-center bg-yellow-200 px-5 py-1">
                        <h3 class="text-">Heure</h3>
                        <h3 class="text-">Horaire</h3>
                    </div>
                    @foreach($day->horaires as $horaire)
                        <div class="w-full flex justify-between items-center bg-gray-100 px-5 py-1">
                            <h3 class="block">{{$horaire->name}}</h3>
                            <div class="">
                                @foreach($horaire->horaire as $hour)
