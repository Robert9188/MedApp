            ...
            <div class="w-full flex justify-center">
                <div class="w-1/3">
                    <div class="">
                        ...
                        <form method="POST" action="{{ route('employee.day.update', $day) }}">
                        @csrf
                        <!--  Name -->
                            <div>
                                <x-label for="name" :value="__('Name')" />

                                <x-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{$day->name}}" required autofocus />
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                >
                                    <option value="{{$day->service->id}}">{{$day->service->name}}</option>
                                    @if($services !== null)
                                        @foreach($services as $service)
                                            @if($service->id !== $day->service->id)
